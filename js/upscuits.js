...
myApp.dashboard = (function($) {
	...
	*/
		...
		while(st = fin.pop()) {
			...
			if (st.len == 1) {
				...
				if (st.right-st.left < 1000*3540) {
					temptip += " ("+new Number((st.right-st.left)/(1000*60)).toFixed(0)+" minutes)";
				} else {
					temptip += " ("+new Number((st.right-st.left)/(1000*3600)).toFixed(1)+" Hour)";
				}
	...
	function Type2Word(t,icon) {
		switch (t) {
			case 1:
				return (icon?"<span class=\"glyphicon glyphicon-remove-sign\"></span> ":"")+"DOWN";
			case 2:
				return (icon?"<span class=\"glyphicon glyphicon-ok-sign\"></span> ":"")+"UP";
			//case 99:
			//	return "未知";
			//case 98:
			//	return "未知";
			default:
				return (icon?"<span class=\"glyphicon glyphicon-question-sign\"></span> ":"")+"?";
		}
	}
